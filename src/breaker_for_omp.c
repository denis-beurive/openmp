...
#pragma omp parallel private(j)
	{
		...
		cdata.initialized=0;
		int first_char_0 = first_char + (loop_size/nb_threads) * rank;
		int last_char_0 = first_char + (loop_size/nb_threads) * (rank+1);

		for(j=1; j<length; j++) tab[j] = first_char;
		tab[0] = first_char_0;


//#pragma omp for
        #pragma omp parallel for default(none) shared(max_iter, cdata, tab, crypted, ret, length, last_char_0, first_char_0, last_char, first_char)
